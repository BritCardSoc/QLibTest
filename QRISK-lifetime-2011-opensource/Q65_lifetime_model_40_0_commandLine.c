...
void usage(void) {
	...
	printf(" * XML: Q65_lifetime_model_40_{cvd,death}_0.xml\n");
	printf(" * STATA dta time stamp: 27 Jul 2010 17:56\n");
	printf(" * C file create date: Tue 27 Jul 2010 22:26:38 BST\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q65_lifetime_model_40_0_commandLine b_AF b_ra b_renal b_treatedhyp b_type2 bmi ethrisk fh_cvd rati sbp smoke_cat town age noOfFollowupYears\n");
}
...
void processFile(char *filename) {
	...
	while (!feof(in)) {
		...
		a_cvd = cvd_female(b_AF,b_ra,b_renal,b_treatedhyp,b_type2,bmi,ethrisk,fh_cvd,rati,sbp,smoke_cat,town,&error,errorBuf,sizeof(errorBuf));
		if (error) {
			printf("%s", errorBuf);
			exit(1);
		}
		a_death = death_female(b_AF,b_ra,b_renal,b_treatedhyp,b_type2,bmi,ethrisk,fh_cvd,rati,sbp,smoke_cat,town,&error,errorBuf,sizeof(errorBuf));
		if (error) {
			printf("%s", errorBuf);
			exit(1);
		}
